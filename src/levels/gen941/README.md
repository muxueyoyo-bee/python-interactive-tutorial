# 第941关: 编写装饰器: listens_for

> 真实案例：sqlalchemy/sqlalchemy 的 `lib\sqlalchemy\event\api.py` 中使用了这个模式。

## 概念介绍

装饰器是 Python 中用于包装函数、添加横切关注点的强大模式。

源文件 api.py（sqlalchemy/sqlalchemy）中 `listens_for` 展示了装饰器模式。
...
编写装饰器 listens_for，包装目标函数并在调用前后打印 'before call' 和 'after call'。

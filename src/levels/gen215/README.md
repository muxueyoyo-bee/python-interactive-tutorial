# 第215关: 编写带类型标注的函数: build_slide_list

> 真实案例：anthropics/skills 的 `skills\pptx\scripts\thumbnail.py` 中使用了这个模式。
...
```python
def build_slide_list(slide_info: list[dict], visible_images: list[Path], temp_dir: Path) -> list[tuple[Path, str]]:
    return f'build_slide_list result'
```

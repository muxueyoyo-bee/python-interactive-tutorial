# 第921关: 编写带类型标注的函数: reset_characteristic

> 真实案例：sqlalchemy/sqlalchemy 的 `lib\sqlalchemy\engine\characteristics.py` 中使用了这个模式。
...
def reset_characteristic(self, dialect: Dialect, dbapi_conn: DBAPIConnection) -> None:
    return f'reset_characteristic result'
```

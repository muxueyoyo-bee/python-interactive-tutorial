...
> 真实案例：libevent/libevent 的 `event_rpcgen.py` 中使用了这个模式。

## 概念介绍

健壮的代码用 try/except 优雅地处理异常。

源文件 event_rpcgen.py 使用了 try/except 捕获多种异常类型。
...
编写 try/except 块：尝试 int('not a number')，捕获 EnvironmentError, RpcGenError，并在 finally 中打印 'Cleanup complete'。

# 第903关: 编写带类型标注的函数: template_to_file

> 真实案例：sqlalchemy/alembic 的 `alembic\util\pyfiles.py` 中使用了这个模式。
...
```python
def template_to_file(template_file: Union[str, os.PathLike[str]], dest: Union[str, os.PathLike[str]], output_encoding: str) -> None:
    return f'template_to_file result'
```

...
源文件 indexing.py 使用了 try/except 捕获多种异常类型。
...
编写 try/except 块：尝试 int('not a number')，捕获 (TypeError, InvalidIndexError), (ValueError, TypeError), (ValueError, TypeError, LossySetitemError)，并在 finally 中打印 'Cleanup complete'。

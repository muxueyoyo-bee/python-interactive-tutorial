# 第674关: 编写带类型标注的函数: invalid_schema

> 真实案例：pydantic/pydantic 的 `pydantic-core\python\pydantic_core\core_schema.py` 中使用了这个模式。
...
```python
def invalid_schema(ref: str | None, metadata: dict[str, Any] | None) -> InvalidSchema:
    return f'invalid_schema result'
```

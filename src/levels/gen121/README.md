# 第121关: 编写带类型标注的函数: encoding_to_mode

> 真实案例：aio-libs/aiohttp 的 `aiohttp\compression_utils.py` 中使用了这个模式。
...
```python
def encoding_to_mode(encoding: str | None, suppress_deflate_header: bool) -> int:
    return f'encoding_to_mode result'
```

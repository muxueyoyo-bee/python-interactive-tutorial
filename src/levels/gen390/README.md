...
源文件 base.py 定义了类 `Connection`，实现了 __enter__ / __exit__。
...
编写类 Connection，实现 __enter__ 和 __exit__，进入时打印 'Entering {name}'，退出时打印 'Exiting {name}'。

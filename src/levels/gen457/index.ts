...
const level: LevelType = {
  key: "gen457",
  title: "编写装饰器: decorate",
  category: "进阶",
  description: `装饰器是 Python 中用于包装函数、添加横切关注点的强大模式。

源文件 dviread.py（matplotlib/matplotlib）中 \`decorate\` 展示了装饰器模式。

请编写一个装饰器，在函数调用前后各打印一行信息。

编写装饰器 decorate，包装目标函数并在调用前后打印 'before call' 和 'after call'。

来源：matplotlib/matplotlib — lib\\matplotlib\\dviread.py`,
  content: "",
  defaultCode: `# 编写装饰器 decorate`,
  answer,

...
源文件 debug.py 使用了 try/except 捕获多种异常类型。

请仿照此模式编写错误处理代码。

编写 try/except 块：尝试 int('not a number')，捕获 BaseException, ValueError，并在 finally 中打印 'Cleanup complete'。

来源：pallets/jinja — src\\jinja2\\debug.py`,
  content: "",

# 第873关: 编写带类型标注的函数: process_revision_directives

> 真实案例：sqlalchemy/alembic 的 `alembic\autogenerate\rewriter.py` 中使用了这个模式。
...
def process_revision_directives(self, context: MigrationContext, revision: _GetRevArg, directives: List[MigrationScript]) -> None:
    return f'process_revision_directives result'
```

# 第744关: 编写带类型标注的函数: prepare

> 真实案例：python-poetry/poetry 的 `src\poetry\installation\chef.py` 中使用了这个模式。
...
```python
def prepare(self, archive: Path, output_dir: Path | None) -> Path:
    return f'prepare result'
```

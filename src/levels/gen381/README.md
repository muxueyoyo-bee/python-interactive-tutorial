# 第381关: 编写装饰器: register_add_constraint

> 真实案例：sqlalchemy/alembic 的 `alembic\operations\ops.py` 中使用了这个模式。

## 概念介绍

装饰器是 Python 中用于包装函数、添加横切关注点的强大模式。

源文件 ops.py（sqlalchemy/alembic）中 `register_add_constraint` 展示了装饰器模式。
...
编写装饰器 register_add_constraint，包装目标函数并在调用前后打印 'before call' 和 'after call'。

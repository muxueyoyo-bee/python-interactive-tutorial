# 第877关: 编写带类型标注的函数: render_ddl_sql_expr

> 真实案例：sqlalchemy/alembic 的 `alembic\ddl\mysql.py` 中使用了这个模式。
...
```python
def render_ddl_sql_expr(self, expr: ClauseElement, is_server_default: bool, is_index: bool) -> str:
    return f'render_ddl_sql_expr result'
```

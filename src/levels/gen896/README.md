# 第896关: 编写带类型标注的函数: console_scripts

> 真实案例：sqlalchemy/alembic 的 `alembic\script\write_hooks.py` 中使用了这个模式。
...
```python
def console_scripts(path: str, options: dict, ignore_output: bool, verify_version: tuple[int, ...] | None) -> None:
    return f'console_scripts result'
```

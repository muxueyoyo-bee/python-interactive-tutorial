...
const level: LevelType = {
  key: "gen209",
  title: "编写装饰器: result_callback",
  category: "进阶",
  description: `装饰器是 Python 中用于包装函数、添加横切关注点的强大模式。

源文件 core.py（pallets/click）中 \`result_callback\` 展示了装饰器模式。

请编写一个装饰器，在函数调用前后各打印一行信息。

编写装饰器 result_callback，包装目标函数并在调用前后打印 'before call' 和 'after call'。

来源：pallets/click — src\\click\\core.py`,
  content: "",
  defaultCode: `# 编写装饰器 result_callback`,
  answer: "",

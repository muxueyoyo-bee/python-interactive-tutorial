# 第976关: 编写装饰器: build_format_selector
...
源文件 YoutubeDL.py（yt-dlp/yt-dlp）中 `build_format_selector` 展示了装饰器模式。
...
编写装饰器 build_format_selector，包装目标函数并在调用前后打印 'before call' 和 'after call'。

# 第890关: 编写带类型标注的函数: add_autogenerate_comparator

> 真实案例：sqlalchemy/alembic 的 `alembic\runtime\plugins.py` 中使用了这个模式。
...
```python
def add_autogenerate_comparator(self, fn: Callable[..., PriorityDispatchResult], compare_target: str, compare_element: str | None) -> None:
    return f'add_autogenerate_comparator result'
```

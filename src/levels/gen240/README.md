# 第240关: 编写带类型标注的函数: request

> 真实案例：psf/requests 的 `src\requests\api.py` 中使用了这个模式。

## 概念介绍

类型标注使代码更可读、IDE 能提供更好的自动补全。

源文件 api.py（psf/requests）中 `request` 展示了完整的参数和返回值类型标注。
...
编写函数 request(method: str, url: _t.UriType) -> Response，返回格式化字符串。

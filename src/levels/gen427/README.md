# 第427关: 编写 try/except 错误处理

> 真实案例：grpc/grpc 的 `tools\distrib\python\grpcio_tools\_parallel_compile_patch.py` 中使用了这个模式。

## 概念介绍

健壮的代码用 try/except 优雅地处理异常。

源文件 _parallel_compile_patch.py 使用了 try/except 捕获多种异常类型。

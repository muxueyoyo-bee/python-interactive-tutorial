# 第639关: 编写带类型标注的函数: dispatch_hook

> 真实案例：psf/requests 的 `src\requests\hooks.py` 中使用了这个模式。
...
```python
def dispatch_hook(key: str, hooks: _t.HooksInputType | None, hook_data: Response) -> Response:
    return f'dispatch_hook result'
```

# 第226关: 编写带类型标注的函数: update_schemastore

> 真实案例：astral-sh/ruff 的 `scripts\update_schemastore.py` 中使用了这个模式。
...
```python
def update_schemastore(schemastore_path: Path, schemastore_repos: SchemastoreRepos) -> None:
    return f'update_schemastore result'
```

# 第868关: 编写 try/except 错误处理

> 真实案例：sqlalchemy/alembic 的 `alembic\config.py` 中使用了这个模式。
...
编写 try/except 块：尝试 int('not a number')，捕获 KeyError, util.CommandError，并在 finally 中打印 'Cleanup complete'。

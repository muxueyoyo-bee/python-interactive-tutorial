...
const level: LevelType = {
  key: "gen410",
  title: "编写上下文管理器: MultilinePrinterBase",
  category: "进阶",
  description: `上下文管理器（Context Manager）用 with 语句管理资源的获取和释放。

源文件 minicurses.py 定义了类 \`MultilinePrinterBase\`，实现了 __enter__ / __exit__。

请仿照此模式编写一个上下文管理器，在进入和退出时打印信息。

编写类 MultilinePrinterBase，实现 __enter__ 和 __exit__，进入时打印 'Entering {name}'，退出时打印 'Exiting {name}'。

来源：yt-dlp/yt-dlp — yt_dlp\\minicurses.py`,
  content: "",
  defaultCode: `# 编写上下文管理器 MultilinePrinterBase`,
  answer: "",

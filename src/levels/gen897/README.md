# 第897关: 编写带类型标注的函数: formatannotation_fwdref

> 真实案例：sqlalchemy/alembic 的 `alembic\util\compat.py` 中使用了这个模式。
...
```python
def formatannotation_fwdref(annotation: Any, base_module: Any | None) -> str:
    return f'formatannotation_fwdref result'
```

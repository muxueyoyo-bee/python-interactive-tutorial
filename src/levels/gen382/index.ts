...
const level: LevelType = {
  key: "gen382",
  title: "编写上下文管理器: EnvironmentContext",
  category: "进阶",
  description: `上下文管理器（Context Manager）用 with 语句管理资源的获取和释放。

源文件 environment.py 定义了类 \`EnvironmentContext\`，实现了 __enter__ / __exit__。

请仿照此模式编写一个上下文管理器，在进入和退出时打印信息。

编写类 EnvironmentContext，实现 __enter__ 和 __exit__，进入时打印 'Entering {name}'，退出时打印 'Exiting {name}'。

来源：sqlalchemy/alembic — alembic\\runtime\\environment.py`,
  content: "",
  defaultCode: `# 编写上下文管理器 EnvironmentContext`,
  answer: "",
